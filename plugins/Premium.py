...
async def remove_premium(client, message):
    if len(message.command) == 2:
        ...
        if await db.remove_premium_access(user_id):
            await message.reply_text(f"Usᴇʀ Rᴇᴍᴏᴠᴇᴅ Sᴜᴄᴄᴇssғᴜʟʟʏ ! Fʀᴏᴍ Pʀᴇᴍɪᴜᴍ Mᴇᴍʙᴇʀsʜɪᴘ\nBᴏᴛ - @{BOT_USERNAME}\n")
            await client.send_message(
                chat_id=user_id,
                text=f"<b>Hᴇʏ {user.mention},\n\nYᴏᴜʀ Pʀᴇᴍɪᴜᴍ Mᴇᴍʙᴇʀsʜɪᴘ Aᴄᴄᴇss Hᴀs Bᴇᴇɴ Rᴇᴍᴏᴠᴇᴅ Fʀᴏᴍ Bᴏᴛ - @{BOT_USERNAME}\nTʜᴀɴᴋ Yᴏᴜ Fᴏʀ Usɪɴɢ Oᴜʀ Pʀᴇᴍɪᴜᴍ Sᴇʀᴠɪᴄᴇs.😊\nCʟɪᴄᴋ Oɴ /plan Tᴏ Cʜᴇᴀᴋ Oᴜᴛ Oᴛʜᴇʀ Pʟᴀɴs.</b>"
            )
        else:
...
async def give_premium_cmd_handler(client, message):
    if len(message.command) == 4:
        ...
        if seconds > 0:
            ...
            expiry = data.get("expiry_time")   
            expiry_str_in_ist = expiry.astimezone(pytz.timezone("Asia/Kolkata")).strftime("%d-%m-%Y\n⏱️ ᴇxᴘɪʀʏ ᴛɪᴍᴇ : %I:%M:%S %p")         
            await message.reply_text(f"ᴘʀᴇᴍɪᴜᴍ ᴀᴅᴅᴇᴅ ꜱᴜᴄᴄᴇꜱꜱꜰᴜʟʟʏ ✅\n\n👤 ᴜꜱᴇʀ : {user.mention}\n⚡ ᴜꜱᴇʀ ɪᴅ : <code>{user_id}</code>\n⏰ ᴘʀᴇᴍɪᴜᴍ ᴀᴄᴄᴇꜱꜱ : <code>{time}</code>\n\n⏳ ᴊᴏɪɴɪɴɢ ᴅᴀᴛᴇ : {current_time}\n\n⌛️ ᴇxᴘɪʀʏ ᴅᴀᴛᴇ : {expiry_str_in_ist}\n\nBᴏᴛ - @{BOT_USERNAME}\n\n𝗬𝗼𝘂𝗿 𝗚𝗿𝗼𝘂𝗽 :-\nhttps://t.me/+IBMieRyDKDEzODJl", disable_web_page_preview=True)
            await client.send_message(
                chat_id=user_id,
                text=f"👋 ʜᴇʏ {user.mention},\nᴛʜᴀɴᴋ ʏᴏᴜ ꜰᴏʀ ᴘᴜʀᴄʜᴀꜱɪɴɢ ᴘʀᴇᴍɪᴜᴍ.\nᴇɴᴊᴏʏ !! ✨🎉\n\n⏰ ᴘʀᴇᴍɪᴜᴍ ᴀᴄᴄᴇꜱꜱ : <code>{time}</code>\n⏳ ᴊᴏɪɴɪɴɢ ᴅᴀᴛᴇ : {current_time}\n\n⌛️ ᴇxᴘɪʀʏ ᴅᴀᴛᴇ : {expiry_str_in_ist}\n\nBᴏᴛ - @{BOT_USERNAME}\n\n𝗬𝗼𝘂𝗿 𝗚𝗿𝗼𝘂𝗽 :-\nhttps://t.me/+IBMieRyDKDEzODJl", disable_web_page_preview=True              
            )    
            await client.send_message(PREMIUM_LOGS, text=f"#Added_Premium\n\n👤 ᴜꜱᴇʀ : {user.mention}\n⚡ ᴜꜱᴇʀ ɪᴅ : <code>{user_id}</code>\n⏰ ᴘʀᴇᴍɪᴜᴍ ᴀᴄᴄᴇꜱꜱ : <code>{time}</code>\n\n⏳ ᴊᴏɪɴɪɴɢ ᴅᴀᴛᴇ : {current_time}\n\n⌛️ ᴇxᴘɪʀʏ ᴅᴀᴛᴇ : {expiry_str_in_ist}\n\nBᴏᴛ - #{BOT_USERNAME}", disable_web_page_preview=True)
...
async def plan(client, message):
    ...
    await message.reply_photo(photo="https://graph.org/file/2dce415ac8d303ee7c7ca.jpg", caption=script.PLAN_TXT.format(message.from_user.mention), reply_markup=InlineKeyboardMarkup(btn))
